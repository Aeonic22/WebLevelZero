    ...
    if (item.isTodo) {
      entry.innerHTML = `
        <div class="todo-row">
          ...
          <div class="todo-body">
            <div class="message-meta">${name2} • ${time}</div>
...
async function fetchAllItems() {
  ...
  items.sort((a, b2) => new Date(b2.timestamp) - new Date(a.timestamp));
  return items;
}
async function refreshMessages() {
  try {
    ...
    const combined = [...plain, ...todos].sort((a, b2) => new Date(b2.timestamp) - new Date(a.timestamp));
...
async function sendMessage(text, extra = {}) {
  const deviceId = getOrCreateDeviceId();
  const name2 = getName();
  console.log(`Attempting to send message from device ${deviceId} with name ${name2}: "${text}"`);
  try {
    console.log("try");
...
function initializeUI() {
  ...
  elements.helloBtn.addEventListener("click", async () => {
    const name2 = getName();
    await sendMessage(`${name2} says hello`);
  });
  elements.eraseBtn.addEventListener("click", async () => {
    const name2 = getName();
    await sendMessage(`${name2} wants to ERASE`);
  });
